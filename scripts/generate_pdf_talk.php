...
function generatePdfForTalk(string $date, string $id = ''): string
{
    ...
    // Institute
    $tex = [
        "\documentclass[12pt]{article}", "\usepackage[margin=15mm,top=3cm,a4paper]{geometry}", "\usepackage[]{graphicx}", "\usepackage[]{wrapfig}", "\usepackage[]{grffile}", "\usepackage[]{amsmath,amssymb}", "\usepackage[hidelinks]{hyperref}", "\usepackage[]{color}", "\usepackage{tikz}", '\linespread{1.15}', '\pagenumbering{gobble}', '\usetikzlibrary{fit,calc,positioning,arrows,backgrounds}', '%\usepackage[sfdefault,light]{FiraSans}', '\usepackage{fontspec}', '\renewcommand\familydefault{\sfdefault}'
