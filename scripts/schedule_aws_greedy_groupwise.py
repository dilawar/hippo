...
def getSpecialization(cur, piOrHost):
    ...
    if a and a is not None:
        return a.get('specialization', 'UNSPECIFIED')
    return 'UNSPECIFIED'
...
def init(cur):
    ...
    for a in cur.fetchall():
        ...
        specialization_[a['login'].strip()] = spec
...
def monthdelta(date, delta):
    ...
    if not m: m = 12
    d = min(date.day, [
        31, 29 if y % 4 == 0 and not y % 400 == 0 else 28, 31, 30, 31, 30, 31,
        31, 30, 31, 30, 31
    ][m - 1])
...
def potentialSpeakerToSwap(speaker,
                           date,
                           candidates,
                           already_swapped=[],
                           low=-21,
...
def print_schedule(schedule, outfile):
    ...
    for date in sorted(schedule):
        ...
        for speaker in schedule[date]:
            line += '%13s %s (%10s, %1d)' % (
                speaker, spec_short(specialization_.get(
                    speaker, 'UNSPECIFIED')),
...
def main(outfile):
    ...
    try:
        ...
        _logger.error("Failed to schedule. Error was %s" % e)
    ...
    for date in ans:
        ...
        for slot in range(3):
            slot = '%s,%d' % (date, slot)
            if slot in g_:
                g_.nodes[slot]['specialization'] = mostCommonSpec
    ...
    try:
        ...
        _logger.warn("Could not write graph to file")
