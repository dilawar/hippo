...
def tomd(htmlfile):
    ...
    pat = re.compile(r'\<div\s+class\s*\=\s*"strip_from_md"\s*\>.+?\</div\>',
                     re.DOTALL)
...
def inlineImageToLink(msg):
    ...
    pat = re.compile(r'data\:image\/(?P<fmt>.+?);base64,(?P<data>.+?=+)',
                     re.DOTALL | re.I)
...
def toTex(infile):
    ...
    scHTML = infile + '.selfcontained.html'
    html = _cmd('pandoc --self-contained -f html -t html %s' % infile)
    html = inlineImageToLink(html)
    with open(scHTML, 'w') as f:
        f.write(html)
    tex = _cmd('pandoc -f html -t latex %s' % scHTML)
    tex = toWrapFigs(tex)
