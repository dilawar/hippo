...
{
    /**
     * @var HTMLPurifier_HTMLDefinition, for easy access
     */
    protected $def;
    ...
    protected function renderChildren($def)
    {
        ...
        if (isset($def->elements)) {
            if ('strictblockquote' == $def->type) {
                $def->validateChildren([], $this->config, $context);
            }
            $elements = $def->elements;
        }
        if ('chameleon' == $def->type) {
            $attr['rowspan'] = 2;
        } elseif ('empty' == $def->type) {
            $elements = [];
        } elseif ('table' == $def->type) {
            $elements = array_flip(
                [
                    'col',
                    'caption',
    ...
    protected function listifyTagLookup($array)
    {
        ...
        foreach ($array as $name => $discard) {
            if ('#PCDATA' !== $name && !isset($this->def->info[$name])) {
                continue;
            }
            $list[] = $name;
        }
