...
{
    $errorMsg = '';
    foreach (['equipment_id', 'date', 'start_time', 'end_time'] as $k) {
...
{
    $errorMsg = '';
    foreach (['equipment_id', 'day_pattern', 'start_time', 'end_time'] as $k) {
        if (!__get__($request, $k, null)) {
            $errorMsg .= "Error: No $k is selected. <br />";
        }
    }
...
trait Lab
{
    ...
    public static function add_inventory_item_helper($data): array
    {
        ...
        $res['status'] = insertOrUpdateTable('inventory', 'id,faculty_in_charge,' . $updatable, $updatable, $data
        );
    ...
    public static function lend_inventory(array $data): array
    {
        ...
        $res = insertOrUpdateTable('borrowing', 'id,inventory_id,borrower,borrowed_on,lender,status,created_on', 'inventory_id,borrower,borrowed_on,lender,status,created_on', $data);
    ...
    public function delete_inventory($itemID)
    {
        ...
        $res = deleteFromTable('inventory', 'id', ['id' => $itemID]);
        if ($res) {
    ...
    public function book_equipment()
    {
        ...
        $res = insertIntoTable('inventory_bookings', 'id,equipment_id,date,start_time,end_time,booked_by,comment', $_POST
            );
    ...
    public function multibook_equipment()
    {
        ...
        foreach ($dates as $date) {
            ...
            if ($eq) {
                ...
                $res = insertIntoTable('inventory_bookings', 'id,equipment_id,vendor,date,start_time,end_time,booked_by,comment', $_POST
                    );
                if ($res) {
    ...
    {
        $res = updateTable('inventory_bookings', 'id', 'status', ['id' => $id, 'status' => 'CACELLED']
        );
        if ($res) {
    ...
    {
        $res = updateTable('inventory_bookings', 'equipment_id,booked_by', 'status', ['equipment_id' => $eid, 'status' => 'CANCELLED', 'booked_by' => whoAmI()]
        );
