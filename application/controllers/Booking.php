...
trait Booking
{
    ...
    public function private_event_edit($arg = '')
    {
        ...
        if ('DO NOTHING' == $response) {
            echo flashMessage('User selected DO NOTHING');
        } elseif ('DELETE EVENT' == $response) {
            $eid = $_POST['eid'];
        ...
        if ('delete' == $action) {
            ...
            if ($res) {
                ...
                $res = updateTable(
                    ...
                    ['external_id' => 'talks.' . $_POST['id'], 'status' => 'CANCELLED', 'last_modified_on' => dbDateTime('now'), 'modified_by' => whoAmI(),
                        ]
                );
                ...
                $res = updateTable(
                    ...
                    ['external_id' => 'talks.' . $_POST['id'], 'status' => 'CANCELLED', 'last_modified_on' => dbDateTime('now'),
                            ]
                );
                redirect('user/show_public');
            ...
            if ($res) {
                ...
                if ($event) {
                    $res = updateTable(
                        ...
                        ['external_id' => 'talks.' . $_POST['id'], 'title' => talkToEventTitle($_POST), 'description' => $_POST['description'], 'url' => __get__($_POST, 'url', ''), 'vc_url' => __get__($_POST, 'vc_url', ''),
                        ]
                    );
    ...
    public function register_talk_submit()
    {
        ...
        if ($missing) {
            ...
            flashMessage(
                "Failed to register your talk. <br />
                You submitted following information:<br /> $msg"
            );
            $this->load_user_view('user_register_talk', ['cFormData' => $_POST]);
        ...
        if ($speaker) {  // Sepeaker is successfully updated. Move on.
            ...
            if ($res2) {
                ...
                $isValid = isBookingRequestValid($_POST);
                if ($isValid['status']) {
                    $date = $_POST['date'];
                    $venue = $_POST['venue'];
