...
class Adminacad extends CI_Controller
{
    ...
    public function load_adminacad_view($view, $data = [])
    {
    ...
    {
        $data = ['course_id' => $course_id, 'year' => $year, 'semester' => $sem];
    ...
    public function update_user()
    {
        ...
        if ($res) {
            ...
            $msg .= arrayToVerticalTableHTML(
                getTableEntry('logins', 'login', ['login' => $login]),
                'profile'
            );
    ...
    public function change_enrollement()
    {
        ...
        } elseif ('audit' == $response) {
            $_POST['type'] = 'AUDIT';
        } elseif ('credit' == $response) {
            $_POST['type'] = 'CREDIT';
        }
    ...
    public function quickenroll()
    {
        ...
        foreach ($enrolls as $i => $en) {
            ...
            if (!in_array($etype, ['AUDIT', 'CREDIT'])) {
    ...
    public function scheduling_request_submit()
    {
        ...
        if ('Reject' == $_POST['response']) {
            ...
            if ($res) {
                ...
                $req = getTableEntry('aws_scheduling_request', 'id', ['id' => $rid]);
    ...
    public function quickgrade()
    {
        ...
        foreach ($gradeCSV as $i => $csv) {
            ...
            // Else assign grade.
            $data = ['student_id' => $login, 'grade' => $grade];
            $data = array_merge($_POST, $data);
    ...
    public function aws_edit_request_action()
    {
        ...
        if ('Reject' == $_POST['response']) {
            ...
            $res = updateTable(
                ...
                'speaker,date',
                ['abstract', 'title', 'is_presynopsis_seminar', 'supervisor_1', 'supervisor_2', 'tcm_member_1', 'tcm_member_2', 'tcm_member_3', 'tcm_member_4',
                        ],
                $req
            );
