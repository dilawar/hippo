...
class Adminservices extends CI_Controller
{
    ...
    public function canteen(string $action = '', string $id = '')
    {
        if (!$action) {
            ...
            // Now show the menu.
            $items = getTableEntries('canteen_menu', 'canteen_name,which_meal,available_from', "status='VALID'");
    ...
    public static function addToCanteenMenu($data)
    {
        ...
        $success = insertOrUpdateTable('canteen_menu', 'id,name,description,price,which_meal,available_from,available_upto'
                . ',canteen_name,day,modified_by', 'price,description,available_from,available_upto,modified_by,status', $data);
    ...
    {
        $success = updateTable('canteen_menu', 'id', 'name,description,price,which_meal,available_from,available_upto'
                . ',canteen_name,day,modified_by', $data);
    ...
    public function manage_canteen(string $action, string $id = '')
    {
        ...
        if ('add' === $action) {
            ...
            foreach ($days as $day) {
                ...
                if (false === strpos($modifiedBy, whoAmI())) {
                    $modifiedBy .= ',' . whoAmI();
                }
            ...
            foreach ($days as $day) {
                $_POST['modified_by'] = implode(',', array_unique(
    ...
    public function add_transport()
    {
        ...
        $_POST['id'] = $id;
        $res = insertIntoTable('transport', 'id,vehicle,vehicle_no,pickup_point,drop_point,day,trip_start_time'
    ...
    {
        var_dump($_POST);
        $res = updateTable('transport', 'id', 'vehicle,vehicle_no,pickup_point,drop_point,day,trip_start_time'
            . ',trip_end_time,comment,status', $_POST);
