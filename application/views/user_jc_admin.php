...
$jcSelect = arrayToSelectList('jc_id', $jcIds, [], false, $jcIds[0]);
...
foreach ($jcIds as $currentJC) {
    ...
    foreach ($subs as $i => $sub) {
        ...
        $presentations = __get__($presentationMap, $login, []);
        ...
        $row = [
            'login' => $login, 'Name' => arrayToName($info) . "<br> $email", 'PI/HOST' => getPIOrHost($login), '#Presentations' => $numPresentations, 'Last Presented On' => humanReadableDate($lastPresentedOn),
