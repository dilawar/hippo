...
$csvdata = ['Speaker,Scheduled on,Last AWS on,Days since last AWS,Total AWS so far'];
...
foreach ($scheduleMap as $date => $schedule) {
    ...
    foreach ($schedule as $i => $upcomingAWS) {
        ...
        $request = getTableEntry(
            ...
            'speaker,status',
            ['status' => 'APPROVED', 'speaker' => $upcomingAWS['speaker']]
        );
