        ...
        if ($awayFrom >= -1 && $awayFrom < 15 * 60) {
            ...
            foreach ($awses as $aws) {
                ...
                foreach ($emails as $email) {
                    ...
                    $email = emailFromTemplate('NOTIFY_SUPERVISOR_TCM_ABOUT_AWS', ['FACULTY' => $name, 'AWS_SPEAKER' => $speaker, 'AWS_DATE' => humanReadableDate($aws['date']), 'AWS_TIME' => humanReadableTime($aws['time']), 'AWS_VENUE' => venueToText($aws['venue'], false), 'AWS_DATE_DB' => $aws['date'],
                    ]
                );
