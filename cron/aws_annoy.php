...
function aws_annoy_cron()
{
    ...
    if ($today > $startDay && $today <= $endDay) {
        ...
        if ($awayFrom > -1 && $awayFrom < 15 * 60) {
            ...
            foreach ($upcomingAws as $aws) {
                ...
                $macros = ['USER' => loginToHTML($aws['speaker']), 'DATE' => humanReadableDate($today),
                ];
    ...
    if (trueOnGivenDayAndTime('today', '10:00')) {
        ...
        if (0 == ($dayNo + 3) % 4) { // Monday and Friday.
            ...
            foreach ($nonConfirmedUpcomingAws as $aws) {
                ...
                $email = emailFromTemplate('REMIND_SPEAKER_TO_CONFIRM_AWS_DATE', ['USER' => loginToHTML($speaker), 'AWS_DATE' => humanReadableDate($aws['date'])]
                );
