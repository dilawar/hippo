...
function eventToEmail(array $event, array $talk = []): array
{
    ...
    $data = emailFromTemplate(
        'this_event', ['EMAIL_BODY' => $talkHTML, 'HOST_INSTITUTE' => strtoupper($hostInstitite),
        ]
    );
...
function coursesToHTMLTable(array $courses, array $runningCourses = null, bool $withFeedbackForm = false, $classes = ''): array
{
    $htmlArr = [];
...
function bookingToHtml(array $booking, $equipmentMap = []): string
{
    if (!$equipmentMap) {
...
{
    $speaker = getTableEntry('speakers', 'id', ['id' => $id]);
...
function hourToHTMLTable($day, $hour, $venue, $section = 4)
{
    ...
    for ($i = 0; $i < $section; ++$i) {
        ...
        if (0 == count($events) && 0 == count($requests)) {
 ...
 * @return
 */
function arrayToRowHTML($array, $tablename, $tobefilterd = '', $linkify = true, $withtr = true)
{
    if ($withtr) {
        $row = '<tr>';
    } else {
...
function arrayToTableHTML(array $array, string $tablename, string $background = '', string $tobefilterd = '', bool $header = true, string $class = '')
{
    if ($background) {
...
function arrayToVerticalTableHTML($array, $tablename, $background = null, $tobefilterd = '', $with_hidden_input = false
) {
    if ($background) {
        $background = "style=\"background:$background;\"";
    } else {
...
function venueSummary($venue, $withName = true)
{
    ...
    if ('REMOTE VC' !== strtoupper(__get__($venue, 'type', 'REMOTE VC'))) {
        $res .= ' [' . $venue['type'] . ']';
    }

    if ('Virtual' !== __get__($venue, 'institute', 'Virtual')) {
        $res .= ', ' . $venue['institute'];
    }
...
function requestToEditableTableHTML($request, $editables = [])
{
 ...
 */
function dbTableToHTMLTable(string $tablename, array $defaults = [], $editables = '', string $button_val = 'submit', string $hide = '', string $classes = '')
{
    global $dbChoices;
    global $useCKEditor;
    ...
    foreach ($schema as $col) {
        ...
        elseif (preg_match("/^set\((.*)\)$/", $ctype, $match)) {
        ...
        // TEXT or MEDIUMTEXT
        elseif (false !== strpos(strtolower($ctype), 'text')) {
 ...
 * @return
 */
function eventToEditableTableHTML($event, $editables = [])
{
    $html = '<table class="request_show_edit">';
 ...
 * @return HTML <select>
 */
function arrayToSelectList(string $name, array $options, $display = [], $multiple_select = false, $selected = '', bool $header = true
    ): string {
    $html = '';
    if (!$multiple_select) {
...
function editableAWSTable(int $awsId = -1, array $default = [], bool $withlogin = false)
{
    ...
    for ($i = 1; $i <= 2; ++$i) {
        ...
        $html .= '
                 ...
                 <td>' . arrayToSelectList(
                     $name, $supervisorIds, $supervisorText, false, $selected
                 );
    ...
    $html .= '
             ...
             <td>' . arrayToSelectList('is_presynopsis_seminar', ['YES', 'NO'], [], false, $selected) . '</td>';
    $html .= '</tr>';
...
function awsToHTMLLarge($aws, $with_picture = true, array $links = [])
{
    ...
    $whereWhen = whereWhenTable($aws['venue'], $aws['date'], $aws['time'], $aws['vc_url'], $inothertable = true);
...
function speakerName($speaker, bool $with_email = false): string
{
    ...
    if ($mname && 'na' != strtolower($mname) && 'notupdated' != strtolower($mname)) {
...
function talkToHTMLLarge($talk, $with_picture = true, string $header = ''): string
{
    ...
    if ($speakerId > 0) {
        $speakerArr = getTableEntry('speakers', 'id', ['id' => $speakerId]);
...
function repeatPatternTable()
{
    ...
    $html .= arrayToMultiCheckbox('week_pattern', 'All,First,Second,Third,Fourth,Fifth', $default = 'All'
                    );
    $html .= '</td>';
    $html .= '<td>
                <nobr>
...
function coursesTable($editable = false, $with_form = true, $class = '')
{
    ...
    foreach ($courses as $c) {
        $instructors = [];
        foreach ($c as $k => $v) {
            if ($v && false !== strpos($k, 'instructor_')) {
...
function gradeSelect($name, $default = 'X')
{
    ...
    $select = arrayToSelectList(
            $name, ['A+', 'A', 'B+', 'B', 'C+', 'C', 'F', 'X'], [], false, $default, false
        );
 ...
 */
/* ----------------------------------------------------------------------------*/
function courseToHTMLRow(array $c, string $slot, string $sem, string $year, array &$enrollments): string
{
    if (!__get__($c, 'id', '')) {
...
function getJCPresenters(array $jc): string
{
    ...
    $others = array_map(function ($x) { return arrayToName(getLoginInfo($x)); }, splitAtCommonDelimeters($jc['other_presenters'])
                );
...
function removeSensitiveInfomation($arr)
{
    ...
    } elseif (__substr__('secret', $id, true)) {
        $res['value'] = 'Not Displayed';
    } else {
        $res['value'] = $arr['value'];
    }
...
function getEnrollmentTableAndEmails($cid, $enrollments, $table_class = 'info')
{
    ...
    foreach (__get__($enrollments, $cid, []) as $r) {
    ...
    return ['html_table' => $table, 'enrolled_emails' => $allEmails];
}
...
function selectYearSemesterForm($defaultYear = '', $defaultSem = '', $endpoint = 'info/courses')
{
    ...
    $yearSelect = arrayToSelectList('year', $years, [], false, $defaultYear);
    $semSelect = arrayToSelectList('semester', ['SPRING', 'AUTUMN'], [], false, $defaultSem);
...
function instructorSpecificQuestion(string $year, string $semester, string $cid, array $q, array $instructors, int $nochangeafater = 1
): string {
    $qid = $q['id'];
...
function courseSpecificQuestion(string $year, string $semester, string $course_id, array $q, int $nochangeafater)
{
    $row = '<tr>';
 ...
 */
/* ----------------------------------------------------------------------------*/
function courseFeedbackForm(string $year, string $semester, string $course_id, array $questions, array $instructors, $nochangeafater = 1
) {
    $html = '';
...
function editHTML($buttonVal)
{
    ...
    $html .= dbTableToHTMLTable('courses_metadata', $course, 'id,credits:required,name:required,description,'
